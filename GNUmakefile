# symlink_example            -*- makefile-gmake -*-
# GNUmakefile
...
dev-install: dune-install
	install -m 555 $(OPAM_SWITCH_PREFIX)/bin/symlink_example ~/bin
.PHONY: home-install

dev-uninstall: 
	$(RM) ~/bin/symlink_example
.PHONY: home-install
...
publish: build mounts
	echo 'url { src:' $$(cat symlink_example.opam | grep dev-repo | awk '{ print $$2 }') '}' >> symlink_example.opam
	make -C /data/web/dldc/opam add NAME=symlink_example OPAM=$$PWD/symlink_example.opam
.PHONY: publish
